...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    for (int i=0; i<5; i++) {
        ChartLabel * label = [[ChartLabel alloc] initWithFrame:CGRectMake(0.0,chartCavanHeight-i*levelHeight+5, UUYLabelwidth, UULabelHeight)];
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    for (int i=0; i<xLabels.count; i++) {
        ChartLabel * label = [[ChartLabel alloc] initWithFrame:CGRectMake((i *  _xLabelWidth ), self.frame.size.height - UULabelHeight, _xLabelWidth, UULabelHeight)];
...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            Bar * bar = [[Bar alloc] initWithFrame:CGRectMake((j+(_yValues.count==1?0.1:0.05))*_xLabelWidth +i*_xLabelWidth * 0.47, UULabelHeight, _xLabelWidth * (_yValues.count==1?0.8:0.45), chartCavanHeight)];
